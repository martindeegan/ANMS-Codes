...
#if CV_MAJOR_VERSION < 4 // If you are using OpenCV 2 or 3
  cv::Mat testImg =
      cv::imread(testImgPath, cv::CV_LOAD_IMAGE_GRAYSCALE); // Read the file
#else
  cv::Mat testImg =
...
#if CV_MAJOR_VERSION < 4 // If you are using OpenCV 2 or 3
  cv::sortIdx(responseVector, Indx, cv::CV_SORT_DESCENDING);
#else
